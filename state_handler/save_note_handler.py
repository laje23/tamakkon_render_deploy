...
async def first_step_save(message):
    ...
    if db_notes.chek_is_exist(int(message.text)):
        ...
        await bale_bot.send_message(
            message.chat.id,
            "این یادداشت موجود است . میخواهید آن را ویرایش کنید",
            edit_menu(),
        )
        return
...
async def next_step_save(message):
    ...
    if not note_number:
        await bale_bot.send_message(
            message.chat.id, "شماره یادداشت پیدا نشد! دوباره امتحان کن.", back_menu()
        )
        message.author.del_state()
        return
    ...
    try:
        db_notes.save_note(note_number, note_text)
        await bale_bot.send_message(
            message.chat.id, "یادداشت با موفقیت ذخیره شد ", back_menu()
        )
